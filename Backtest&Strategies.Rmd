...
```{r}
signals <- c("signal_MACD", "signal_RSI", "signal_KD", "signal_OBV")
all_signals <- cbind(comp_list$SNPS[,c("Date","Close")],comp_list$SNPS[,signals])
signals_comb <- c("signal_MACD", "signal_RSI", "signal_OBV") #"signal_KD"

Combo <- cbind(Date = all_signals[,"Date"],
               Close = all_signals[,"Close"], all_signals[,signals_comb])

Signal_combined <- function(data){
  
  data[, signals_comb[1:2]][data[, signals_comb[1:2]] == -1] <- 0
  data$signal_comb <- rep(0, length(Combo$signal_MACD))

  data$signal_comb <- ifelse(rowSums(data[, signals_comb], na.rm = TRUE) == 0, 0,
...
#### 我試著回測但figure不出要怎用
```{r}
a <- Signal_combined(Combo)
a <- replace(a, is.na(a), 0)
a
signals <- names(a)[-c(1:2)]
signals
signal_return(a, start_date, signals)
