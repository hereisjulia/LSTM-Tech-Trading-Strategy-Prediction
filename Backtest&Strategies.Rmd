...
MSFT_data <- comp_list$MSFT
View(SNPS_data)
unique(SNPS_data$signal_MACD)
unique(SNPS_data$signal_RSI)
unique(SNPS_data$signal_KD)
```
...
signal_return <- function(data, start_date, signal_name){
  ...
  annualized_return <- round((initial_capital/latest_capital)^(365/as.numeric((max(data$Date)-min(data$Date))))-1, 4)*100
